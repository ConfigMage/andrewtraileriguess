...
- In Print setup, enter your machine's usable length, width, height, and extrusion width. The app checks whether the unrotated shell fits as one piece and whether wall, roof, and floor thickness reach one entered extrusion width. It does not split the shell into parts. Toggle the reference box in the 3D scene or clear the setup. Machine values stay in this browser and are not included in design JSON.
...
Print fit excludes the chassis and tongue. It does not assess rotation, supports, tool access, material behavior, or whether the current visual structure is printable. The checks are planning aids, not validated manufacturing requirements.
