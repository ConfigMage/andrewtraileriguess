...
- In Print setup, enter your machine's usable length, width, and height. The app plans bounded sections across both walls, roof, floor, and ends. It shows a total and per-surface count, seam lines, opening cut warnings, and a selected section's dimensions and surface patch. Select a count to jump to that surface or use the section number and arrows. Whole-shell fit and extrusion-width checks remain under the details control. Machine values stay in this browser and are not included in design JSON.
...
Print planning excludes the chassis and tongue. The cut plan is a visual subdivision with conservative bounding boxes, not separate printable solids. It does not define joints, allowances, rotation, supports, tool access, material behavior, or whether the visual structure is printable. The checks are planning aids, not validated manufacturing requirements.
